...
const _get_specific_device = async (gw_id) => {
...
const _insert_device = async (device) => {
  let _device = await _get_specific_device(device.gw_id);
  if (_device) {
...
const _remove_specific_device = async (gw_id) => {
  try {
    await admin_model.update({ role: 'admin' },
      { '$pull': { devices: { gw_id: _gw_id } } });
    return true;
  } catch (e) {
    return false;
  }
